...
## 7. 下一步行動計畫 (Next Steps)

1.  **實體電路檢查與重焊：**
    *   移除中央匯流點的熱熔膠。
    *   確認並重新焊接：GPIO 4 藍線、9.4k 上拉電阻 (接往 3.3V)、以及 5 顆分壓電阻。
    *   使用三用電表確認各節點間的導通性 (逼逼檔測試)。
2.  **韌體數值驗證：**
    *   硬體修復後，使用 `calibration.ino` 進行空載測試，確認讀數穩定在 **4095**。
    *   依序放入 1 號至 5 號杯，記錄每個獨立杯子的 ADC 數值。
    *   如果需要，根據實測 ADC 數值微調 `website_control.ino` 中的 `R_WEIGHTS` 及 `R_PULLUP` 參數，以達到最精準的解碼效果。
